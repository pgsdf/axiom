...
pub const BootstrapManager = struct {
    ...
    fn writeBootstrapMetadata(self: *Self, staging_dir: []const u8, packages: []const BootstrapPackage, options: ExportOptions) !void {
        ...
        // Write YAML header
        try std.fmt.format(writer,"# Axiom Bootstrap Tarball Metadata\n", .{});
        try std.fmt.format(writer,"# Generated by: axiom bootstrap export\n", .{});
        try std.fmt.format(writer,"---\n", .{});
        try std.fmt.format(writer,"format_version: 1\n", .{});
        try std.fmt.format(writer,"created: \"{s}\"\n", .{try self.getCurrentTimestamp()});
        try std.fmt.format(writer,"os_version: \"{s}\"\n", .{os_version});
