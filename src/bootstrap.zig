...
pub const BootstrapManager = struct {
    ...
    fn writeBootstrapMetadata(self: *Self, staging_dir: []const u8, packages: []const BootstrapPackage, options: ExportOptions) !void {
        ...
        // Write YAML header
        try writer.writeAll("# Axiom Bootstrap Tarball Metadata\n");
        try writer.writeAll("# Generated by: axiom bootstrap export\n");
        try writer.writeAll("---\n");
        try writer.writeAll("format_version: 1\n");
        {
            var buf: [256]u8 = undefined;
            const str = std.fmt.bufPrint(&buf, "created: \"{s}\"\n", .{try self.getCurrentTimestamp()}) catch unreachable;
            try writer.writeAll(str);
        }
        {
            var buf: [256]u8 = undefined;
            const str = std.fmt.bufPrint(&buf, "os_version: \"{s}\"\n", .{os_version}) catch unreachable;
            try writer.writeAll(str);
        }
        {
            var buf: [256]u8 = undefined;
            const str = std.fmt.bufPrint(&buf, "arch: \"{s}\"\n", .{arch}) catch unreachable;
            try writer.writeAll(str);
        }
        {
            var buf: [256]u8 = undefined;
            const str = std.fmt.bufPrint(&buf, "description: \"{s}\"\n", .{description}) catch unreachable;
            try writer.writeAll(str);
        }
        try writer.writeAll("\n");
        ...
        for (packages) |pkg| {
            {
                var buf: [256]u8 = undefined;
                const str = std.fmt.bufPrint(&buf, "  - name: \"{s}\"\n", .{pkg.name}) catch unreachable;
                try writer.writeAll(str);
            }
            {
                var buf: [256]u8 = undefined;
                const str = std.fmt.bufPrint(&buf, "    version: \"{s}\"\n", .{pkg.version}) catch unreachable;
                try writer.writeAll(str);
            }
            if (pkg.origin) |origin| {
                var buf: [256]u8 = undefined;
                const str = std.fmt.bufPrint(&buf, "    origin: \"{s}\"\n", .{origin}) catch unreachable;
                try writer.writeAll(str);
            }
