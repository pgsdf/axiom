...
pub const BundleManifest = struct {
    ...
    pub fn serialize(self: BundleManifest, allocator: std.mem.Allocator) ![]const u8 {
        ...
        try std.fmt.format(writer,"format_version: {d}\n", .{self.format_version});
        ...
        try std.fmt.format(writer,"entry_point: {s}\n", .{self.entry_point});
        try std.fmt.format(writer,"uncompressed_size: {d}\n", .{self.uncompressed_size});
        ...
        if (self.signature) |sig| {
            try std.fmt.format(writer,"signature: {s}\n", .{sig});
        }
