...
pub const BundleManifest = struct {
    ...
    pub fn serialize(self: BundleManifest, allocator: std.mem.Allocator) ![]const u8 {
        ...
        const str1 = std.fmt.bufPrint(&buf1, "format_version: {d}\n", .{self.format_version}) catch unreachable;
        try writer.writeAll(str1);
        try writer.writeAll("name: ");
        try writer.writeAll(self.name);
        try writer.writeAll("\n");
        var buf2: [64]u8 = undefined;
        const str2 = std.fmt.bufPrint(&buf2, "version: {d}.{d}.{d}\n", .{
            self.version.major,
            self.version.minor,
            self.version.patch,
        }) catch unreachable;
        ...
        const str3 = std.fmt.bufPrint(&buf3, "created_at: {d}\n", .{self.created_at}) catch unreachable;
        try writer.writeAll(str3);
        try writer.writeAll("entry_point: ");
        try writer.writeAll(self.entry_point);
        try writer.writeAll("\n");
        var buf4: [64]u8 = undefined;
        const str4 = std.fmt.bufPrint(&buf4, "uncompressed_size: {d}\n", .{self.uncompressed_size}) catch unreachable;
        try writer.writeAll(str4);
        var buf5: [64]u8 = undefined;
        const str5 = std.fmt.bufPrint(&buf5, "compression: {s}\n", .{@tagName(self.compression)}) catch unreachable;
        try writer.writeAll(str5);

        try writer.writeAll("packages:\n");
        for (self.packages) |pkg| {
            var buf6: [128]u8 = undefined;
            const str6 = std.fmt.bufPrint(&buf6, "  - {s}@{d}.{d}.{d}\n", .{
                pkg.name,
                pkg.version.major,
                pkg.version.minor,
                pkg.version.patch,
...
pub const BundleBuilder = struct {
    ...
    fn generateAppRunScript(
        ...
        for (packages) |pkg| {
            var buf_ld: [256]u8 = undefined;
            const str_ld = std.fmt.bufPrint(&buf_ld,
                \\LD_LIBRARY_PATH="${{LD_LIBRARY_PATH}}:${{APPDIR}}/packages/{s}/root/lib"
                \\
            , .{pkg.name}) catch unreachable;
            try writer.writeAll(str_ld);
        }
        ...
        for (packages) |pkg| {
            ...
            const str_path = std.fmt.bufPrint(&buf_path,
                \\PATH="${{PATH}}:${{APPDIR}}/packages/{s}/root/bin"
                \\
            , .{pkg.name}) catch unreachable;
            try writer.writeAll(str_path);
        }

        var buf_exec: [512]u8 = undefined;
        const str_exec = std.fmt.bufPrint(&buf_exec,
            \\export PATH
            \\
            \\# Execute the application
            \\exec "${{APPDIR}}/packages/{s}/root/bin/{s}" "$@"
            \\
        , .{ packages[0].name, entry_point }) catch unreachable;
        try writer.writeAll(str_exec);
